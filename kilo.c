...
#define INV_COLORS "\x1b[7m" // http://vt100.net/docs/vt100-ug/chapter3.html#SGR
#define NORM_COLORS "\x1b[m" // http://vt100.net/docs/vt100-ug/chapter3.html#SGR
#define DEFAULT_FG                                                             \
  "\x1b[39m" // https://en.wikipedia.org/wiki/ANSI_escape_code#Select_Graphic_Rendition_parameters
  ...
  for (int y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      ...
      for (j = 0; j < len; j++) {
        if (iscntrl(c[j])) {
          char sym = (c[j] <= 26) ? '@' + c[j] : '?';
          abAppend(ab, INV_COLORS, 4);
          abAppend(ab, &sym, 1);
          abAppend(ab, NORM_COLORS, 3);
          if (current_color != -1) {
            char buf[16];
...
void editorRefreshScreen(void) {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cy - E.rowoff) + 1,
           (E.rx - E.coloff) + 1); // modified CURS_TOP that includes offset
